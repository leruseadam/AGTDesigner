#!/usr/bin/env python3
"""
Enhanced Auto Upload to PythonAnywhere
...
def calculate_file_priority(filename):
    ...
    # Bonus for recent files (within last 7 days)
    return priority
...
def main():
    """Main function."""
    print("🔄 Enhanced Auto Upload to PythonAnywhere")
    print("=" * 50)
    ...
    print(f"\n🤔 Do you want to upload '{file_path.name}' to PythonAnywhere?")
    print("This will replace the current default file on the web server.")
    
    try:
        response = input("Enter 'y' to continue or 'n' to cancel: ").lower().strip()
