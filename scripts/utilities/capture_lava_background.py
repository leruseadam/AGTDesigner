#!/usr/bin/env python3
"""
Capture a static JPEG of the lava lamp background for promo purposes.
This script uses Playwright to render the canvas and capture it as an image.
"""
...
def create_standalone_html():
    """Create a standalone HTML file that renders the lava lamp."""
    html_content = '''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Lava Lamp Background Capture</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
(function() {
  ...
  const fragSrc = `
    ...
    uniform vec3 u_blobPos[BLOB_COUNT];
    uniform vec3 u_blobVel[BLOB_COUNT];
    ...
    void main() {
      ...
      vec2 detailUV = uv * 0.02 + u_time * 0.00005;
      float surfaceDetail = fbm(detailUV) * 0.003;
      color += vec3(surfaceDetail * 0.001);
      
      float shadow = smoothstep(0.0, 0.3, depth) * 0.35;
      color *= (1.0 - shadow);
      
      float organicNoise = fbm(uv * 0.01 + u_time * 0.00002);
      color += vec3(organicNoise * 0.0005);
  ...
  let frameCount = 0;
  const CAPTURE_FRAME = 120; // Capture after 2 seconds at 60fps

  function animate() {
    const t = performance.now() * 0.001;
    const dt = Math.min(0.016, t - (blobs[0].lastUpdate || t));
    ...
    for (let i = 0; i < BLOB_COUNT; i++) {
      ...
      const gravity = 0.000001 * blob.mass;
      const drag = 0.9998;
      const buoyancy = 0.0000003 * blob.mass;
      
      const organicForceX = Math.sin(t * 0.0002 + blob.movePhase + i) * 0.0000002;
      const organicForceY = Math.cos(t * 0.00015 + blob.movePhase + i) * 0.0000001;
      ...
      blob.depth += Math.sin(t * 0.0005 + i) * 0.000001;
      blob.depth = Math.max(0.3, Math.min(1.0, blob.depth));
      
      const pulse = Math.sin(t * 0.0008 + blob.pulsePhase + i) * 0.0002;
      const organicPulse = Math.cos(t * 0.0006 + i) * 0.0001;
  ...
  animate();
    ...
    html_path = Path(__file__).parent / 'lava_lamp_capture.html'
    with open(html_path, 'w') as f:
        f.write(html_content)
    return html_path

def capture_canvas(output_path='lava_background.jpg', width=1920, height=1080):
    """Capture the lava lamp canvas as a JPEG."""
    ...
    try:
        ...
        html_path = create_standalone_html()
        abs_html_path = html_path.resolve()
        
        print(f"Rendering lava lamp at {width}x{height}...")
        ...
        with sync_playwright() as p:
            ...
            # Wait for the canvas to render (wait for captureReady flag)
            page.wait_for_function('window.captureReady === true', timeout=10000)
            
            # Additional wait to ensure rendering is complete
            page.wait_for_timeout(500)
            
            # Get the canvas element
            canvas = page.query_selector('#lava-lamp-canvas')
            
            if canvas:
                # Capture the canvas as a screenshot
                canvas.screenshot(path=output_path, type='jpeg', quality=95)
                print(f"✓ Successfully captured lava background to: {output_path}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Capture lava lamp background as JPEG')
    parser.add_argument('--output', '-o', default='lava_background.jpg',
                       help='Output JPEG filename (default: lava_background.jpg)')
    ...
    success = capture_canvas(args.output, args.width, args.height)
