#!/bin/bash
# Fix upload timing issue with status tracking
echo "Fixing upload timing issue..."

# Create backup
cp /home/adamcordova/AGTDesigner/app.py /home/adamcordova/AGTDesigner/app.py.backup.$(date +%Y%m%d_%H%M%S)

# Create improved app.py with better timing
...
if [ $? -eq 0 ]; then
    ...
    echo "✅ Fixed upload timing issue!"
    echo "✅ Added detailed logging!"
    echo "✅ Initialize status before processing!"
    echo "Reloading web app..."
    touch /var/www/www_agtpricetags_com_wsgi.py
    echo "Web app reloaded! Upload timing should be fixed now."
else
...
echo "Upload timing fix applied successfully!"
