...
class ProductDatabase:
    ...
    def init_database(self):
        ...
        with self._init_lock:
            ...
            try:
                ...
                # Check if products table exists (PostgreSQL)
                cursor.execute("""
                    SELECT EXISTS (
                ...
                # Create strains table (PostgreSQL)
                cursor.execute('''
    ...
    def _migrate_database_schema_safe(self, cursor, conn):
        ...
        try:
            ...
            # Check if tables exist and have data (PostgreSQL)
            cursor.execute("""
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_schema = 'public' 
        ...
        try:
            ...
            for index, row in filtered_df.iterrows():
                try:
                    ...
                    # Additional validation: Fill missing vendor/type with defaults
                    ...
                    if not vendor or str(vendor).lower() in ['nan', 'none', 'null', '']:
                        ...
                        logger.warning(f"Row {index + 1}: Product '{product_name}' missing vendor, set to 'Unknown Vendor'")
                    if not product_type or str(product_type).lower() in ['nan', 'none', 'null', '']:
                        ...
                        logger.warning(f"Row {index + 1}: Product '{product_name}' missing product type, set to 'Unknown Type'")
    ...
    def export_database(self, output_path: str):
        ...
        try:
            ...
            # Get available columns dynamically to avoid SQL errors for both SQLite and PostgreSQL
            product_columns = set()
            try:
                # SQLite
                ...
                try:
                    ...
                    logger.error(f"Failed to discover product columns: {meta_err}")
                    product_columns = set()
            
            # Desired output columns grouped by theme; we'll include only those present
            desired_columns_in_order = [
                ...
                # Compatibility alternates sometimes used in legacy sheets
                'ProductName', 'Joint Ratio', 'Quantity Received*', 'qty'
            ]
            
            # Use the same approach as get_all_products for consistency
            # Build a safe, quoted select list based on actually present columns
            selected_product_cols = [col for col in desired_columns_in_order if col in product_columns]
            # Ensure essential columns if present
            essential_cols = ['Product Name*', 'Product Type*', 'Vendor/Supplier*']
            if not any(c in product_columns for c in essential_cols):
                logger.warning("None of the essential product columns found; export may be empty")

            if selected_product_cols:
                quoted_cols = ", ".join([f'p."{c}"' for c in selected_product_cols])
                query = f'SELECT p.id, {quoted_cols} FROM products p ORDER BY p.id'
            else:
                # Fallback to at least exporting IDs if no recognized columns
                query = 'SELECT p.id FROM products p ORDER BY p.id'
            ...
            # Convert results to dicts dynamically based on selected columns
            for row in results:
                product: dict = {'id': row[0]}
                for index, col_name in enumerate(selected_product_cols, start=1):
                    product[col_name] = row[index]
    ...
    def _add_missing_columns_safe(self, cursor, conn):
        ...
        try:
            ...
            # Check if we've already run this migration to avoid repeated attempts (PostgreSQL)
            cursor.execute("""
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_schema = 'public' 
            ...
            """)
            migration_log_exists = cursor.fetchone()[0]
            ...
            # Check and add missing columns to strains table (PostgreSQL)
            cursor.execute("""
                SELECT column_name 
                FROM information_schema.columns 
                WHERE table_name = 'strains' AND table_schema = 'public'
            """)
            existing_strain_columns = {row[0] for row in cursor.fetchall()}
    ...
    def find_best_product_match(self, product_name: str, vendor: str = None, product_type: str = None, strain: str = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if normalized_name:
                # Build more specific search conditions to avoid random matches
                conditions = []
                condition_params = []
                
                # Strategy 1: Exact or very close matches first
                conditions.append("(p.\"Product Name*\" = ? OR p.\"Product Name*\" LIKE ?)")
                condition_params.extend([product_name, f"{product_name}%"])
                
                # Strategy 2: Remove "Medically Compliant" prefix and try again
                if product_name.startswith("Medically Compliant - "):
                    simplified_name = product_name.replace("Medically Compliant - ", "").strip()
                    conditions.append("(p.\"Product Name*\" = ? OR p.\"Product Name*\" LIKE ?)")
                    condition_params.extend([simplified_name, f"{simplified_name}%"])
                
                # Strategy 3: Look for products that contain the main product name (without weight/size info)
                # Extract the core product name (remove weight, size, and common suffixes)
                import re
                core_name = re.sub(r'\s*-\s*\d+\.?\d*\s*(g|oz|ml|pk|ea|each)\s*$', '', product_name)
                core_name = re.sub(r'\s*-\s*\d+\.?\d*\s*(g|oz|ml|pk|ea|each)\s*$', '', core_name)  # Remove multiple weight patterns
                if core_name != product_name and len(core_name) > 10:  # Only if we actually removed something meaningful
                    conditions.append("(p.\"Product Name*\" LIKE ? OR p.\"Description\" LIKE ?)")
                    condition_params.extend([f"%{core_name}%", f"%{core_name}%"])
                
                # Strategy 4: Look for products with similar product type/category
                if product_type:
                    # Map JSON inventory types to database product types
                    type_mapping = {
                        'Marijuana Mix Infused': ['Infused', 'Mix', 'Edible', 'Gummy', 'Chocolate', 'Candy'],
                        'Marijuana Flower': ['Flower', 'Bud', 'Cannabis', 'Marijuana'],
                        'Marijuana Concentrate': ['Concentrate', 'Wax', 'Shatter', 'Resin', 'Oil', 'Cartridge'],
                        'Marijuana Vape': ['Vape', 'Cartridge', 'Pen', 'Disposable'],
                        'Marijuana Pre-roll': ['Pre-roll', 'Joint', 'Cigarette', 'Roll']
                    }
                    
                    if product_type in type_mapping:
                        type_conditions = []
                        for type_term in type_mapping[product_type]:
                            type_conditions.append("(p.\"Product Name*\" LIKE ? OR p.\"Product Type*\" LIKE ?)")
                            condition_params.extend([f"%{type_term}%", f"%{type_term}%"])
            ...
            # Order by relevance (exact matches first, then partial matches, then by ID)
            query += " ORDER BY CASE WHEN p.\"Product Name*\" = ? THEN 1 WHEN p.\"Product Name*\" LIKE ? THEN 2 WHEN p.\"Product Name*\" LIKE ? THEN 3 ELSE 4 END ASC, p.id DESC LIMIT 1"
            params.extend([product_name, f"{product_name}%", f"%{normalized_name}%"])
