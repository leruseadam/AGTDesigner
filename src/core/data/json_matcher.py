...
class JSONMatcher:
    ...
    def __init__(self, excel_processor):
        ...
        # Cache for database products to avoid repeated queries (especially slow on PythonAnywhere)
        self._cached_db_products = None
        self._db_products_cache_timestamp = None
        self._db_products_cache_ttl = 300  # Cache for 5 minutes
    ...
    def _reset_product_database_cache(self):
        ...
        if self._product_db_instance:
            try:
                ...
                pass
        self._product_db_instance = None
        # Also clear cached database products when store changes
        self._cached_db_products = None
        self._db_products_cache_timestamp = None
        # Also clear cached database products when store changes
        self._cached_db_products = None
        ...
        try:
            ...
            try:
                ...
                cache_valid = (
                    ...
                    (current_time - self._db_products_cache_timestamp) < self._db_products_cache_ttl
                )
                
                if cache_valid:
                    db_products = self._cached_db_products
                    cache_age = current_time - self._db_products_cache_timestamp
                    logging.debug(f"Using cached database products ({len(db_products)} products, cached {cache_age:.1f}s ago)")
                else:
                    ...
                    logging.debug(f"Loaded and cached {len(db_products)} products from DATABASE")
