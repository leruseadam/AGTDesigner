...
def extract_products_from_manifest(manifest_json):
    ...
    for item in items:
        ...
        if not product.get('Product Name*') and not product.get('ProductName'):
            product['Product Name*'] = (item.get('product_name', '') or 
                                      item.get('inventory_name', '') or 
                                      item.get('name', '') or 
                                      item.get('description', '') or 
                                      'Unknown Product').strip()
        ...
        if not product.get('Product Brand') and not product.get('ProductBrand'):
            product['Product Brand'] = (item.get('brand', '') or 
                                      item.get('vendor', '') or 
        ...
        if not product.get('Price') and not product.get('Price*'):
            product['Price'] = (item.get('price', '') or 
        ...
        if not product.get('Product Type*') and not product.get('ProductType'):
            product['Product Type*'] = (item.get('inventory_type', '') or 
                                       item.get('product_type', '') or 
                                       item.get('inventory_category', '') or 
                                       item.get('category', '') or 
                                       item.get('C', '') or  # Column C mapping
                                       'Edible (Solid)').strip()
        ...
        if not product.get('Weight*'):
            product['Weight*'] = (item.get('weight', '') or 
                                 item.get('unit_weight', '') or 
                                 item.get('net_weight', '') or 
                                 item.get('F', '') or  # Column F mapping
                                 '').strip()
            logging.info(f"🔍 DEBUG: Fallback weight: '{product['Weight*']}'")
        
        if not product.get('Product Strain'):
            product['Product Strain'] = (item.get('strain_name', '') or 
                                        item.get('strain', '') or 
                                        item.get('strain_type', '') or 
                                        item.get('E', '') or  # Column E mapping
                                        '').strip()
            logging.info(f"🔍 DEBUG: Fallback strain: '{product['Product Strain']}'")
...
class JSONMatcher:
    ...
    def _create_product_from_json_item(self, item: Dict, global_vendor: str) -> Dict:
        ...
        try:
            ...
            weight = str(item.get("unit_weight", item.get("weight", ""))).strip()
            weight_units = str(item.get("unit_weight_uom", item.get("uom", "g"))).strip()
            price = str(item.get("line_price", item.get("price", ""))).strip()
            strain = str(item.get("strain_name", item.get("strain", ""))).strip()
            ...
            logging.warning(f"Error creating product from JSON item: {e}")
            return {}
    ...
    def fetch_and_match_with_product_db(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                response.raise_for_status()
                payload = response.json()
                logging.info(f"✅ Successfully fetched JSON data directly from URL")
            except (requests.exceptions.RequestException, ValueError) as direct_error:
                logging.info(f"Direct request failed, trying proxy: {direct_error}")
                # Fallback to proxy endpoint if direct request fails
                import os
                ...
                try:
                    ...
                    response.raise_for_status()
                    payload = response.json()
                    logging.info(f"✅ Successfully fetched JSON data via proxy")
                except Exception as proxy_error:
                    logging.error(f"❌ Both direct and proxy requests failed: {proxy_error}")
                    return []
                
            # Handle both list and dictionary payloads with enhanced validation
            if isinstance(payload, list):
                items = payload
                global_vendor = ""
                logging.info(f"✅ Processing JSON list with {len(items)} items")
            elif isinstance(payload, dict):
                items = payload.get("inventory_transfer_items", [])
                # Extract global vendor from document metadata
                global_vendor = payload.get("from_license_name", "")
                logging.info(f"✅ Processing JSON dict with {len(items)} items, global vendor: {global_vendor}")
                
                # Additional validation for common JSON structures
                if not items and "items" in payload:
                    items = payload.get("items", [])
                    logging.info(f"✅ Found items in 'items' field: {len(items)} items")
                if not items and "products" in payload:
                    items = payload.get("products", [])
                    logging.info(f"✅ Found items in 'products' field: {len(items)} items")
            else:
                logging.warning(f"❌ Unexpected payload type: {type(payload)}")
                return []
            ...
            for i, item in enumerate(items):
                if isinstance(item, dict):
                    ...
                    logging.warning(f"⚠️ Skipping non-dict item at index {i}: {type(item)}")
            
            if not valid_items:
                logging.warning("❌ No valid dictionary items found in JSON")
                return []
                
            items = valid_items
            logging.info(f"✅ Processing {len(items)} valid JSON items")
