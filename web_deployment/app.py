...
# Fix for CodeRunner virtual environment issues
# Force deactivate any problematic virtual environments
if 'VIRTUAL_ENV' in os.environ and 'numpy' in os.environ.get('VIRTUAL_ENV', ''):
    # If we're in a virtual environment with numpy issues, try to use system Python
    try:
        import subprocess
        # Try to restart with system Python
        subprocess.run(['/usr/bin/python3'] + sys.argv, check=True)
        sys.exit(0)
    except:
        pass  # Continue with current Python if restart fails
...
def reset_excel_processor():
    ...
    try:
        if cache is not None:
            # Clear all cache keys that might contain stale data
            cache_keys_to_clear = [
                'available_tags', 'selected_tags', 'filter_options', 'dropdowns',
                'json_matched_tags', 'full_excel_tags', 'initial_data',
                'file_path', 'full_excel_cache_key', 'json_matched_cache_key'
            ]
            ...
            for cache_key_name in cache_keys_to_clear:
                try:
                    ...
                    cache_keys_to_try = [
                        ...
                        f"{cache_key_name}_{session.get('session_id', 'default')}",
                        f"session_{session.get('session_id', 'default')}_{cache_key_name}"
                    ]
            ...
            logging.info("Aggressively cleared all cache keys")
...
def cleanup_databases():
    """Cleanup all database connections to prevent locks."""
    global _product_database
    try:
        if _product_database:
            _product_database.cleanup()
            _product_database = None
...
def create_app():
    ...
    def cleanup_databases_on_teardown(error):
        """Cleanup database connections when request context is torn down."""
        try:
            cleanup_databases()
        except Exception as e:
...
# Commented out automatic initialization to prevent database locks during startup
...
def get_session_product_database():
    ...
    try:
        if not hasattr(app, '_product_database'):
            from src.core.data.product_database import ProductDatabase
            # CRITICAL FIX: Use the correct database path
            db_path = os.path.join(current_dir, 'products.db')
...
def _enhance_json_with_excel_data(json_tag, excel_product):
    ...
    # Priority fields: JSON data takes precedence for these fields
    json_priority_fields = [
        'Product Name*', 'ProductName', 'Vendor', 'Product Brand', 
        'Price', 'Weight*', 'Weight', 'Quantity*', 'Quantity'
    ]
    
    # Excel priority fields: Excel data takes precedence for these fields
    excel_priority_fields = [
        'Lineage', 'Product Type*', 'Product Strain', 'Description',
        'THC test result', 'CBD test result', 'Test result unit (% or mg)',
        'Room*', 'State', 'Is Sample? (yes/no)', 'Is MJ product?(yes/no)',
        'Discountable? (yes/no)', 'Medical Only (Yes/No)', 'DOH'
    ]
    
    # Fill missing fields from Excel data
    for field in excel_priority_fields:
        if field in excel_product and excel_product[field] and (field not in enhanced_tag or not enhanced_tag[field]):
            enhanced_tag[field] = excel_product[field]
    
    # Fill missing fields from JSON data
    for field in json_priority_fields:
        if field in json_tag and json_tag[field] and (field not in enhanced_tag or not enhanced_tag[field]):
            enhanced_tag[field] = json_tag[field]
    
    # Add any additional Excel fields that don't exist in JSON
    for field, value in excel_product.items():
        if field not in enhanced_tag and value:
            enhanced_tag[field] = value
    
    # Ensure we have a proper display name
    if 'displayName' not in enhanced_tag or not enhanced_tag['displayName']:
        product_name = enhanced_tag.get('Product Name*', enhanced_tag.get('ProductName', ''))
        vendor = enhanced_tag.get('Vendor', enhanced_tag.get('Product Brand', ''))
...
def upload_file_simple_pythonanywhere():
    ...
    try:
        ...
        try:
            ...
            import pandas as pd
            try:
                # Load with minimal rows and processing (increased for large files)
                df = pd.read_excel(temp_path, nrows=25000, engine='openpyxl')  # Increased to 25000 rows
                if not df.empty:
                    processor.df = df
                    success = True
...
def process_excel_sync(filename, temp_path):
    ...
    try:
        ...
        import pandas as pd
        try:
            # Try to load with row limit first (increased for large files)
            df = pd.read_excel(temp_path, nrows=25000, engine='openpyxl')
            if not df.empty:
                processor.df = df
                success = True
                logging.info(f"[ULTRA-FAST-BG] Loaded {len(df)} rows (limited to 25000)")
            else:
                success = False
        except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if product_db and hasattr(product_db, 'store_excel_data'):
                ...
                # The minimal_load_file method may rename columns which breaks database storage
                import pandas as pd
    ...
    try:
        ...
        try:
            undo_response = requests.post(
                f"http://127.0.0.1:{app.config.get('PORT', 5003)}/api/save-selection-state",
...
def generate_labels():
    try:
        ...
        if selected_tags_to_use:
            ...
            # DEBUG: Add debug info to error response
            debug_info = {
                'selected_tags_from_request': selected_tags_from_request,
                'session_selected_tags': session.get('selected_tags', []),
                'session_json_selected_tags': session.get('json_selected_tags', []),
                'session_last_json_match_count': session.get('last_json_match_count', 0),
                'session_json_matched_cache_key': session.get('json_matched_cache_key'),
                'excel_processor_selected_tags': getattr(excel_processor, 'selected_tags', 'NOT SET'),
                'excel_processor_selected_tags_count': len(getattr(excel_processor, 'selected_tags', [])),
            }
            logging.warning("No selected_tags provided in request body or session")
            return jsonify({
                'error': 'No tags selected. Please select at least one tag before generating labels.',
        ...
        # CRITICAL FIX: Check if this is a JSON matched session early
        json_matched_cache_key = session.get('json_matched_cache_key')
        is_json_matched_session = json_matched_cache_key is not None
        
        # CRITICAL FIX: If no session cache key, check if we have JSON matched products in cache
        if not is_json_matched_session:
            # Try to find any JSON matched cache entries
            import time
            current_time = int(time.time())
            for i in range(10):  # Check last 10 seconds
                test_key = f"json_matched_{session.get('session_id', 'default')}_{current_time - i}"
                if cache.get(test_key):
                    json_matched_cache_key = test_key
                    is_json_matched_session = True
                    session['json_matched_cache_key'] = json_matched_cache_key
                    session.modified = True
                    break
        
        # DEBUG: Log session detection
        print(f"DEBUG: json_matched_cache_key = {json_matched_cache_key}")
        print(f"DEBUG: is_json_matched_session = {is_json_matched_session}")
        print(f"DEBUG: has_database = {has_database}")
        print(f"DEBUG: excel_processor.selected_tags = {getattr(excel_processor, 'selected_tags', 'NOT SET')}")
        print(f"DEBUG: session.selected_tags = {session.get('selected_tags', 'NOT SET')}")
        print(f"DEBUG: session.last_json_match_count = {session.get('last_json_match_count', 'NOT SET')}")
        logging.info(f"DEBUG: json_matched_cache_key = {json_matched_cache_key}")
        logging.info(f"DEBUG: is_json_matched_session = {is_json_matched_session}")
        logging.info(f"DEBUG: has_database = {has_database}")
        logging.info(f"DEBUG: session.selected_tags = {session.get('selected_tags', 'NOT SET')}")
        logging.info(f"DEBUG: session.last_json_match_count = {session.get('last_json_match_count', 'NOT SET')}")
        
        # CRITICAL FIX: For JSON matched sessions, update session.selected_tags with actual JSON matched product names
        if is_json_matched_session:
            json_matched_products = cache.get(json_matched_cache_key)
            if json_matched_products:
                # Override session selected_tags with actual JSON matched product names
                ...
                for product in json_matched_products:
                    if isinstance(product, dict):
                        ...
                        if product_name:
                            actual_selected_tags.append(product_name)
                
                # Update session with actual selected tags from JSON matched products
                session['selected_tags'] = actual_selected_tags
                excel_processor.selected_tags = actual_selected_tags
                logging.info(f"CRITICAL FIX: Updated session.selected_tags with {len(actual_selected_tags)} actual JSON matched product names")
                logging.info(f"CRITICAL FIX: First few actual tags: {actual_selected_tags[:5]}")
                print(f"CRITICAL FIX: Updated session.selected_tags with {len(actual_selected_tags)} actual JSON matched product names")
                print(f"CRITICAL FIX: First few actual tags: {actual_selected_tags[:5]}")
            else:
                logging.warning("CRITICAL FIX: JSON matched session but no products found in cache")
                print("CRITICAL FIX: JSON matched session but no products found in cache")
        
        # First, try to get records from database (preferred source) - but skip for JSON matched sessions
        if has_database and not is_json_matched_session:
                    ...
                    if db_records:
                        ...
                        for db_record in db_records:
                            ...
                            record = {
                                ...
                                'DescAndWeight': db_record.get('Description', ''),  # Use Description for DescAndWeight
                            ...
                            print(f"DEBUG: Final record DescAndWeight (using Description): '{record.get('DescAndWeight', '')}'")
        ...
        # CRITICAL FIX: For JSON matched sessions, use JSON matched products directly from cache
        logging.info(f"DEBUG: is_json_matched_session = {is_json_matched_session}")
        logging.info(f"DEBUG: json_matched_cache_key = {json_matched_cache_key}")
        
        if is_json_matched_session:
            logging.info("CRITICAL FIX: JSON matched session - using JSON matched products directly from cache")
            json_matched_products = cache.get(json_matched_cache_key)
            if json_matched_products:
                logging.info(f"CRITICAL FIX: Found {len(json_matched_products)} JSON matched products in cache")
                # Convert JSON matched products to records for template processing
                records = []
                for product in json_matched_products:
                    if isinstance(product, dict):
                        # Try to get full database record for this product
                        product_name = product.get('Product Name*', product.get('ProductName', ''))
                        full_db_record = None
                        
                        if product_name and has_database:
                            try:
                                from src.core.data.product_database import get_product_database
                                product_db = get_product_database()
                                if product_db:
                                    ...
                                    if not db_records:
                                        ...
                                        db_records = product_db.get_products_by_names_fuzzy([product_name])
                                    
                                    if db_records:
                                        full_db_record = db_records[0]
                                        logging.info(f"✅ Enhanced JSON matched product with full database record: {product_name} -> {full_db_record.get('Product Name*', 'Unknown')}")
                                    else:
                                        logging.warning(f"⚠️ No database record found for {product_name}, using JSON matched product data")
                            except Exception as e:
                                logging.warning(f"Failed to get full database record for {product_name}: {e}")
                        
                        # Use full database record if available, otherwise use JSON matched product
                        if full_db_record:
                            record = full_db_record.copy()
                            # Preserve JSON matching metadata
                            record['Source'] = product.get('Source', 'JSON Match (Database)')
                            record['Match Method'] = product.get('Match Method', '')
                            record['Match Score'] = product.get('Match Score', '')
                            record['Original JSON Product Name'] = product.get('Original JSON Product Name', '')
                            # CRITICAL FIX: Preserve original database description instead of overriding it
                            # Only set description if it doesn't exist in the database record
                            if not record.get('Description'):
                                record['Description'] = f"Matched from database - {record.get('Product Name*', product_name)}"
                            # Ensure Product Strain fields exist for generator logic
                            if 'Product Strain' not in record:
                                record['Product Strain'] = record.get('ProductStrain', '')
                            if 'ProductStrain' not in record:
                                record['ProductStrain'] = record.get('Product Strain', '')
                            # Normalize Price for generator
                            if 'Price' not in record or not record.get('Price'):
                                record['Price'] = record.get('Price* (Tier Name for Bulk)', '')
                            # Force DescAndWeight to be Description plus weight when available
                            try:
                                desc_text = record.get('Description', '') or ''
                                # Prefer separate Weight* and Units
                                weight_val = (record.get('Weight*', '') or record.get('CombinedWeight', '') or '').strip()
                                units_val = (record.get('Units', '') or record.get('Weight Unit* (grams/gm or ounces/oz)', '') or '').strip()
                                if weight_val and units_val:
                                    weight_units_val = f"{str(weight_val)}{str(units_val)}"
                                else:
                                    combined_val = units_val or weight_val
                                    weight_units_val = (str(combined_val).replace(' ', '')) if combined_val else ''
                                record['DescAndWeight'] = f"{desc_text} - {weight_units_val}" if desc_text and weight_units_val else desc_text
                            except Exception:
                                record['DescAndWeight'] = record.get('Description', '')
                        else:
                            # Fallback to JSON matched product data
                            record = {
                                ...
                                'Weight Unit* (grams/gm or ounces/oz)': product.get('Weight Unit* (grams/gm or ounces/oz)', ''),
                                'Lineage': product.get('Lineage', ''),
                                'Product Strain': product.get('Product Strain', ''),
                                'ProductStrain': product.get('Product Strain', ''),
                                'Description': product.get('Description', ''),
                                'Price* (Tier Name for Bulk)': product.get('Price* (Tier Name for Bulk)', ''),
                                'Price': product.get('Price', product.get('Price* (Tier Name for Bulk)', '')),
                                'Source': product.get('Source', ''),
                                'Match Method': product.get('Match Method', ''),
                                'Match Score': product.get('Match Score', ''),
                                'Original JSON Product Name': product.get('Original JSON Product Name', ''),
                                ...
                                'THC': product.get('THC', ''),
                                'CBD': product.get('CBD', ''),
                                'AI': product.get('Total THC', ''),
                                'AJ': product.get('THCA', ''),
                                'AK': product.get('CBDA', ''),
                                'ProductVendor': product.get('Vendor/Supplier*', ''),
                                'Quantity Received*': product.get('Quantity Received*', ''),
                                'Barcode': product.get('Barcode*', ''),
                                'Quantity': product.get('Quantity*', '')
                            }
                        
                        records.append(record)
                logging.info(f"CRITICAL FIX: Converted {len(records)} JSON matched products to records")
            else:
                logging.warning("CRITICAL FIX: No JSON matched products found in cache")
                records = []
        else:
            if is_json_matched_session and not has_excel_data:
                logging.warning("CRITICAL FIX: JSON matched session but no Excel data available!")
            elif not is_json_matched_session:
                logging.info("DEBUG: Not a JSON matched session, using normal processing")
        
        # Fallback to Excel data if database didn't provide records (for non-JSON sessions)
        if not records and has_excel_data:
            logging.info("Using Excel data for record generation (fallback)")
            
            # If still no records, try Excel processor
            if not records:
        ...
        if not records:
            # DEBUG: Add debug info to error response
            debug_info = {
                'json_matched_cache_key': json_matched_cache_key,
                'is_json_matched_session': is_json_matched_session,
                'has_database': has_database,
                'has_excel_data': has_excel_data,
                'excel_processor_selected_tags': getattr(excel_processor, 'selected_tags', 'NOT SET'),
                'excel_processor_selected_tags_count': len(getattr(excel_processor, 'selected_tags', [])),
                'valid_selected_tags_count': len(valid_selected_tags) if 'valid_selected_tags' in locals() else 0
            }
            logging.error("No selected tags found in the data or failed to process records.")
            return jsonify({
                'error': 'No selected tags found in the data or failed to process records. Please ensure you have selected tags and they exist in the loaded data.',
                'debug_info': debug_info
            }), 400
...
def get_available_tags():
    try:
        ...
        if check_only:
            logging.info("Available tags check-only request")
        ...
        try:
            ...
            if not test_cursor.fetchone():
                ...
                if current_store:
                    ...
                    from src.core.data.product_database import ProductDatabase
                    main_db_path = os.path.join(current_dir, 'products.db')
...
@app.route('/api/force-clear-cache', methods=['POST'])
def force_clear_cache():
    """Force clear ALL caches and data - more aggressive than regular clear-cache"""
    try:
        logging.info("=== FORCE CLEARING ALL CACHES AND DATA ===")
        
        # Clear all global variables
        global _initial_data_cache, _cache_timestamp, _excel_processor, _excel_processor_reset_flag
        _initial_data_cache = None
        _cache_timestamp = None
        _excel_processor = None
        _excel_processor_reset_flag = True
        ...
        if cache is not None:
            cache.clear()
            logging.info("Force cleared Flask cache")
        
        # Clear session data completely
        session.clear()
        session.modified = True
        logging.info("Force cleared session data")
        
        # Clear processing status
        global processing_status, processing_timestamps
        processing_status.clear()
        processing_timestamps.clear()
        logging.info("Force cleared processing status")
        
        # Force garbage collection
        import gc
        gc.collect()
        logging.info("Force garbage collection completed")
        
        # Clear any remaining caches in Excel processor
        try:
            excel_processor = get_excel_processor()
            if excel_processor:
                ...
                if hasattr(excel_processor, 'dropdown_cache'):
                    excel_processor.dropdown_cache.clear()
                ...
                if hasattr(excel_processor, 'selected_tags'):
                    excel_processor.selected_tags = []
                logging.info("Force cleared Excel processor caches")
        except Exception as e:
            logging.warning(f"Error clearing Excel processor caches: {e}")
        
        return jsonify({
            'success': True,
            'message': 'All caches and data force cleared successfully'
        })
        
    except Exception as e:
        logging.error(f"Error force clearing cache: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def json_match():
    ...
    try:
        ...
        # Perform fast JSON matching for better performance
        matched_products = json_matcher.fetch_and_match_fast(url)
        logging.info(f"Fast JSON matching returned {len(matched_products) if matched_products else 0} products")
        
        if not matched_products:
            return jsonify({'error': 'No products found in JSON data'}), 400
        
        # CRITICAL FIX: Store JSON matched products in session cache for generation
        import time
        json_matched_cache_key = f"json_matched_{session.get('session_id', 'default')}_{int(time.time())}"
        cache.set(json_matched_cache_key, matched_products, timeout=3600)  # 1 hour timeout
        session['json_matched_cache_key'] = json_matched_cache_key
        session['last_json_match_count'] = len(matched_products)
        session['json_match_timestamp'] = time.time()
        session.modified = True
        
        # Also store in Excel processor for immediate access
        # CRITICAL FIX: Store product names (strings) in selected_tags, not full dictionaries
        product_names = [p.get('Product Name*', p.get('ProductName', '')) for p in matched_products if isinstance(p, dict)]
        excel_processor.selected_tags = product_names
        session['selected_tags'] = product_names
        
        # CRITICAL FIX: Force session to be saved and mark as modified
        session.modified = True
        session.permanent = True
        
        logging.info(f"CRITICAL FIX: Stored {len(matched_products)} JSON matched products in cache with key: {json_matched_cache_key}")
        logging.info(f"CRITICAL FIX: Set selected_tags to {len(product_names)} product names: {product_names[:3]}...")
...
@app.route('/test_products_large.json')
def serve_test_products_large():
    """Serve the large test products JSON file for testing."""
    try:
        with open('test_products_large.json', 'r') as f:
            data = json.load(f)
        return jsonify(data)
    except FileNotFoundError:
        return jsonify({'error': 'Large test products file not found'}), 404
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/test_products_same_vendor.json')
def serve_test_products_same_vendor():
    """Serve the same vendor test products JSON file for testing."""
    try:
        with open('test_products_same_vendor.json', 'r') as f:
            data = json.load(f)
        return jsonify(data)
    except FileNotFoundError:
        return jsonify({'error': 'Same vendor test products file not found'}), 404
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/cultivera_test.json')
def serve_cultivera_test():
    """Serve the real Cultivera test products JSON file for testing."""
    try:
        with open('cultivera_test.json', 'r') as f:
            data = json.load(f)
        return jsonify(data)
    except FileNotFoundError:
        return jsonify({'error': 'Cultivera test products file not found'}), 404
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def clear_all_cache():
    """Clear all caches and session data to reset the application state."""
    try:
        # Clear all caches
        cache.clear()
        ...
        if hasattr(app, '_json_matcher'):
            delattr(app, '_json_matcher')
        if hasattr(app, '_excel_processor'):
            delattr(app, '_excel_processor')
        if hasattr(app, '_product_database'):
            delattr(app, '_product_database')
        
        logging.info("All caches and session data cleared")
        return jsonify({
            'success': True,
            'message': 'All caches and session data cleared successfully'
        })
...
@app.route('/api/fix-database-lineage', methods=['POST'])
def fix_database_lineage():
    """Fix lineage values for nonclassic product types in the database."""
    try:
        product_db = get_product_database()
        result = product_db.fix_nonclassic_lineage()
        ...
        logging.error(f"Error fixing database lineage: {e}")
        return jsonify({
            'success': False,
            'error': str(e),
            'message': 'Failed to fix database lineage'
        }), 500
