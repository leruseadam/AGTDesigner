...
class JSONMatcher:
    ...
    def fetch_and_match(self, url: str) -> List[Dict]:
        ...
        try:
            ...
            # VENDOR PROPAGATION FIX: Apply global vendor to items missing vendor info
            # This fixes the "4/49 sucks" problem where advanced_matcher.py skips NO_VENDOR items
            if unique_items:
                # Step 1: Find any item with vendor information (prefer global_vendor first)
                discovered_vendor = None
                if global_vendor and global_vendor.strip():
                    discovered_vendor = global_vendor.strip()
                    logging.info(f"🔍 Using document-level vendor for propagation: '{discovered_vendor}'")
                else:
                    # Look for any item with vendor info
                    for item in unique_items:
                        item_vendor = item.get('vendor', '').strip()
                        if item_vendor and item_vendor != 'NO_VENDOR':
                            discovered_vendor = item_vendor
                            logging.info(f"🔍 Found item-level vendor for propagation: '{discovered_vendor}'")
                            break
                
                # Step 2: Apply discovered vendor to all items missing vendor info
                if discovered_vendor:
                    propagated_count = 0
                    for item in unique_items:
                        current_vendor = item.get('vendor', '').strip()
                        if not current_vendor or current_vendor == 'NO_VENDOR':
                            item['vendor'] = discovered_vendor
                            propagated_count += 1
                    
                    logging.info(f"📈 Vendor propagation: Applied '{discovered_vendor}' to {propagated_count} items")
                    ...
                    logging.info(f"📊 Post-propagation vendor distribution: {vendor_counts}")
                    
                    no_vendor_remaining = vendor_counts.get('NO_VENDOR', 0)
                    if no_vendor_remaining == 0:
                        logging.info(f"✅ SUCCESS: Vendor propagation eliminated ALL NO_VENDOR items!")
                        logging.info(f"🎯 This should fix the advanced_matcher.py vendor filtering bottleneck!")
                    else:
                        logging.warning(f"⚠️ {no_vendor_remaining} items still show NO_VENDOR after propagation")
                else:
                    logging.warning(f"⚠️ No vendor information found for propagation - all items may be skipped by vendor filtering")
            ...
            # NO DEDUPLICATION: Every JSON item results in a tag, even if identical
            logging.info(f"NO DEDUPLICATION: {len(matched_products)} products processed. Every JSON item generates a tag.")
    ...
    def _create_product_from_json(self, json_item, global_vendor):
        """Create a product object from JSON data, always checking for a database match."""
        try:
            ...
            # Always check for a database match for every JSON item
            from .product_database import ProductDatabase
            product_db = ProductDatabase()
            db_matches = self._find_similar_products_in_database(product_name, vendor, brand, product_type, strain, weight, product_db)
            if db_matches:
                db_match = db_matches[0]
                logging.info(f"🔗 Matched JSON product '{product_name}' to database entry '{db_match.get('Product Name*', db_match.get('product_name', ''))}'")
                return db_match
            else:
                logging.warning(f"❌ No database match found for JSON product '{product_name}'. Falling back to inference.")

            # Fallback: Use inference from similar database matches
            inferred_data = self._infer_from_similar_database_matches(product_name, vendor, brand, product_type, strain, weight)
